...
func Mine(ctx context.Context, addr address.Address, h host.Host, api v1api.FullNode, db db.DB, cfg *Config) error {
	...
	for {
		...
		case <-ctx.Done():
			...
			return nil
		case toMir := <-m.ToMir:
			base, err := m.StateManager.api.ChainHead(m.ctx)
			if err != nil {
				log.Errorf("unable to get base for mpool select")
				continue
			}

			nextHeight := base.Height() + 1
			fmt.Println(">>>> SELECTING MESSAGES FROM MPOOL FROM BASE: ", base.Height())
			...
			fmt.Println(">>>>>>> SENDING BATCH TO MIR")
			toMir <- requests
